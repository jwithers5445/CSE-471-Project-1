...
bool CSynthesizer::Generate(double* frame)
{
    ...
    if (m_compression)
    {
        ...
        m_effects.Compression(frame, coframe);
    ...
    if (m_noisegate)
    {
        ...
        m_effects.NoiseGate(frame, nframe);
...
void CSynthesizer::XmlLoadInstrument(IXMLDOMNode* xml)
{
    ...
    for (; node != NULL; NextNode(node))
    {
        ...
        else if (name == L"compressionoff")
        {
            m_compression = false;
        }
        else if (name == L"NoiseGate")
        {
            m_noisegate = true;
        }
        else if (name == L"NoiseGateOff")
        {
            m_noisegate = false;
        }
    }
